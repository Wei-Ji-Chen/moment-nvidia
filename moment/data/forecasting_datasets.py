...
from moment.utils.data import upsample_timeseries, downsample_timeseries
...
def filter_epidemic_datasets(
    ...
    return [
        d for d in datasets_collection 
        if dataset_name in d and indicator in d
    ]
...
class ShortForecastingDataset(TaskDataset):
    ...
    def _split_long_timeseries(self, df):
        ...
        chunks = np.array_split(timeseries, indices_or_sections=len(timeseries)/self.seq_len)
    ...
    def _list_of_long_timeseries(self, df):
        return ["solar_4_seconds_dataset", 
    ...
    def __getitem__(self, index):
        ...
        return TimeseriesData(
                ...
                forecast=forecast.reshape((self.n_channels, self.forecast_horizon)) if forecast is not None else None,
                input_mask=input_mask,
                name=self.dataset_name,
                metadata=metadata,
            )
...
class EpidemicForecastingDataset(TaskDataset):
    ...
    def _check_inputs(self):
        assert self.data_split in ['train', 'test', 'val'],\
            "data_split must be one of 'train', 'test' or 'val'"
        assert self.task_name in ['forecasting', 'pre-training', 'imputation'],\
