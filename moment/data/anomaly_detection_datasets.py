...
class AnomalyDetectionDataset(TaskDataset):
    ...
    def _check_inputs(self):
        ...
        assert self.task_name in ['anomaly-detection', 'pre-training'],\
    ...
    def __repr__(self):
        repr = f"AnomalyDetectionDataset(dataset_name={self.dataset_name}," + \
    ...
    def _downsample_timeseries(self, timeseries, labels):
        # Downsampling code taken from TSADAMS: XXXX
        if (self.downsampling_factor is not None) and (self.min_length is not None) and (len(timeseries)//self.downsampling_factor > self.min_length):
            padding = self.downsampling_factor - len(timeseries)%self.downsampling_factor
            timeseries = np.pad(timeseries, ((padding, 0)))
            labels = np.pad(labels, (padding, 0))

            timeseries = timeseries.reshape(timeseries.shape[-1]//self.downsampling_factor, self.downsampling_factor).max(axis=1)
            labels = labels.reshape(labels.shape[0]//self.downsampling_factor, self.downsampling_factor).max(axis=1)
    ...
    def __getitem__(self, index):
        ...
        if self.task_name == 'anomaly-detection':
            ...
            return TimeseriesData(
                    ...
                    labels=self.labels[seq_start:seq_end].astype(
                        int).reshape((self.n_channels, self.seq_len)),
    ...
    def plot(self, idx):
        ...
        if self.task_name == 'anomaly-detection':
            plt.plot(np.arange(self.seq_len), 
                     labels, label='Labels', c='red', linestyle='--')
