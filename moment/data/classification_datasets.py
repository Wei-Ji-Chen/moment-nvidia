...
from moment.utils.data import \
    upsample_timeseries, downsample_timeseries, interpolate_timeseries
from moment.data.load_data import load_from_tsfile
from .base import TaskDataset, TimeseriesData, DataSplits
...
def get_classification_datasets(collection : str ="UCR", subset : Optional[str] = None):
    ...
    datasets = [dataset for dataset in datasets if dataset.split('/')[-1] not in remove_datasets]
    if subset == "univariate":
        summary = pd.read_csv("~/MOMENT/assets/data/summaryUnivariate.csv")
        # TODO: This would not work in general
        univariate_datasets = summary.problem.tolist()
        datasets = [i for i in datasets if i.split("/")[-2].strip() in univariate_datasets]
    return datasets
...
class ClassificationDataset(TaskDataset):
    def __init__(self, 
        ...
        scaling_type : str
            Type of scaling. One of 'local' or 'global'.
            Global scaling standard scales the entire dataset. 
            Local scaling standard scales each time-series. 
        task_name : str
    ...
    def _check_inputs(self):
        ...
        assert self.task_name in ['classification', 'pre-training'],\
    ...
    def __repr__(self):
        repr = f"ClassificationDataset(dataset_name={self.dataset_name}," + \
            f"n_timeseries={self.length_timeseries}," + \
            f"dataset_size={self.__len__()}," + \
            f"length_of_each_timeseries={self.length_each_timeseries}," + \
    ...
    def _get_borders(self):
        ...
        if self.data_split in ['train', 'val']:
            train_ratio, val_ratio = self.train_ratio / (self.train_ratio + self.val_ratio), self.val_ratio / (self.train_ratio + self.val_ratio)
            train_end = int(train_ratio*self.length_timeseries)
        
        return DataSplits(train=slice(0, train_end), 
                          val=slice(train_end, None), 
                          test=slice(0, None))
    ...
    def _read_data(self) -> TimeseriesData:
        ...
        self.length_each_timeseries = self.data.shape[2]
        ...
        if self.scale and self.dataset_name not in DATASETS_WITHOUT_NORMALIZATION:
            ...
            self.data = self.data.reshape(n_lines, -1, self.data.shape[-1])
        
        # Get the number of channels
        self.n_channels = self.data.shape[1] if len(self.data.shape) == 3 else 1
        
        if self.output_type == 'univariate':    
            # Duplicate labels for multivariate datasets
            self.labels = np.repeat(self.labels.reshape(-1, 1), self.n_channels, axis=1).reshape(-1, 1)
            self.data = self.data.reshape(-1, self.data.shape[-1])
            self.n_channels = 1
    ...
    def __getitem__(self, index):
        ...
        labels = self.labels[index, ].astype(int) if self.task_name == 'classification' else None
        
        if timeseries_len <= self.seq_len:
            timeseries, input_mask =\
            ...
            timeseries, input_mask =\
                downsample_timeseries(timeseries,
                                      self.seq_len,
    ...
    def _check_and_remove_nans(self):
        if np.isnan(self.data).any():
            ...
            self.data = interpolate_timeseries(
                ...
                interp_length=self.data.shape[-1])
    ...
    def _check_if_equal_length(self):
        if isinstance(self.data, list):
            ...
            logging.info(f"Time-series have unequal lengths. Reshaping to {self.data.shape}")
