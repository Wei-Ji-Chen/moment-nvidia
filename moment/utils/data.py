...
def interpolate_timeseries(timeseries : npt.NDArray, 
                           interp_length : int = 512) -> npt.NDArray:
...
def upsample_timeseries(timeseries : npt.NDArray, 
                        seq_len : int, 
...
def downsample_timeseries(timeseries : npt.NDArray, 
                          seq_len : int, 
                          sampling_type : str = 'interpolate'):
        input_mask = np.ones(seq_len)
        if sampling_type == "last":
            ...
            timeseries, input_mask = \
                upsample_timeseries(
                    timeseries, seq_len, sampling_type="pad", 
                    direction="forward")       
        else:
