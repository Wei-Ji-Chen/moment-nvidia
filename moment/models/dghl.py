"""
Copyright XXXX-15.com, Inc. or its affiliates. All Rights Reserved.
...
class DGHL(BaseModel):    
    ...
    def _preprocess_batch_hierarchy(self, windows):
        ...
        windows = windows.reshape(batch_size*self.a_L, self.n_features, self.sub_window_size)
        # (batch_size*a_L, n_features, sub_window_size)
    ...
    def forward(self, Y, mask, **kwargs):
        ...
        z_0 = self._get_initial_z(p_0_chains_l=self.p_0_chains_l, p_0_chains_u=self.p_0_chains_u,
                                  z_persistent=self.z_persistent, batch_size=batch_size)

        # Sample z with Langevin Dynamics
        z, _, _ = self.infer_z(z=z_0, Y=Y, mask=mask, n_iters=self.z_iters, with_noise=self.z_with_noise)
        ...
        return DGHLOutputs(
            timeseries=Y, reconstruction=Y_hat, mask=mask)
    ...
    def training_step(self, x_enc, input_mask, **kwargs):
        ...
        loss = self.compute_loss(
            outputs.timeseries, outputs.reconstruction)
