...
from moment.utils.utils import get_huggingface_model_dimensions, get_anomaly_criterion
...
class MOMENT(nn.Module):
    ...
    def _update_inputs(self, configs: Namespace | dict, **kwargs) -> NamespaceWithDefaults:
    ...
    def _get_head(self, task_name: str) -> nn.Module:
        if task_name in {
            ...
            return ClassificationHead(
                ...
                (task_name == TASKS.SHORT_HORIZON_FORECASTING and \
                 self.configs.finetuning_mode != 'zero-shot'):
            num_patches = (
    ...
    def _get_patchtst_encoder(self, configs):
        ...
        encoder = Encoder(
            [
                EncoderLayer(
                    AttentionLayer(
                        FullAttention(attention_dropout=configs.attention_dropout,
    ...
    def embed(
        ...
        **kwargs
        ...
        attention_mask = Masking.convert_seq_to_patch_view(
            input_mask, self.patch_len).repeat_interleave(n_channels, dim=0)
        outputs = self.encoder(inputs_embeds=enc_in,
                    attention_mask=attention_mask) 
        enc_out = outputs.last_hidden_state
            
        enc_out = enc_out.reshape(
            (-1, n_channels, n_patches, self.configs.d_model)) 
        ...
        return TimeseriesOutputs(embeddings=enc_out, 
    ...
    def pretraining(self, 
        ...
        """
        # TODO: Remove mask as a parameter
        ...
        if self.configs.transformer_type == "encoder_decoder":
            outputs = self.encoder(
                inputs_embeds=enc_in, decoder_inputs_embeds=enc_in,
                attention_mask=attention_mask)
        else:                      
            outputs = self.encoder(inputs_embeds=enc_in,
                        attention_mask=attention_mask) 
        ...
        illegal_output = self._check_model_weights_for_illegal_values()\
            if self.configs.debug else None
                
        return TimeseriesOutputs(input_mask=input_mask, 
    ...
    def initialize_soft_prompt(self, **kwargs):
        ...
        self.soft_prompt = nn.Embedding(n_soft_prompt_tokens, 
                                        self.configs.d_model)
        return self.soft_prompt

    def _cat_learned_embedding_to_input(self, 
                                        prompt_embeds,
                                        enc_in) -> torch.Tensor:
    ...
    def _extend_attention_mask(self, 
                               attention_mask,
                               n_tokens):
        n_batches = attention_mask.shape[0]
        extension = torch.full((n_batches, n_tokens), 1).to(
            self.configs.device)
        return torch.cat(
            [extension, attention_mask], dim=1)
    ...
    def reconstruct(self, 
        ...
        attention_mask = Masking.convert_seq_to_patch_view(
            input_mask, self.patch_len).repeat_interleave(
                n_channels, dim=0).to(x_enc.device)
        ...
        if 'prompt_embeds' in kwargs:
            ...
            enc_in = self._cat_learned_embedding_to_input(
                prompt_embeds, enc_in)
            
            attention_mask = self._extend_attention_mask(
                attention_mask, n_tokens)
    ...
    def short_forecast(self, 
                       ...
                       **kwargs):
        # TODO: Make sure to take care mask as input
        # mask would be mask tokens which are attended to
        # and input_mask is typically unattended
        # TODO: Aspirational Add support for soft prompt tokens
    ...
    def forward(self, 
        ...
        elif self.task_name == TASKS.LONG_HORIZON_FORECASTING or (self.task_name == TASKS.SHORT_HORIZON_FORECASTING and self.configs.finetuning_mode != 'zero-shot'):
    ...
    def _check_model_weights_for_illegal_values(self):
        illegal_encoder_weights = torch.stack(
            [torch.isnan(p).any() for p in self.encoder.parameters()]).any().item()
        illegal_head_weights = torch.stack(
            [torch.isnan(p).any() for p in self.head.parameters()]).any().item()
        illegal_patch_embedding_weights = torch.stack(
            [torch.isnan(p).any() for p in self.patch_embedding.parameters()]).any().item()
