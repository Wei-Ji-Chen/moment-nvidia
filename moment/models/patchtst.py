...
class MOMENT(nn.Module):
    def __init__(self, configs, **kwargs):
        ...
        self.tokenizer = Patching(patch_len=configs.patch_len, 
                                  stride=configs.patch_stride_len)
        self.patch_embedding = PatchEmbedding(
            configs.d_model, configs.patch_len, configs.patch_stride_len, 
            padding, configs.dropout, configs.orth_gain).to(configs.device)
        ...
        # Prediction Head
        self.head_nf = configs.d_model * ((max(configs.seq_len, configs.patch_len) - configs.patch_len) // configs.patch_stride_len + 1)
