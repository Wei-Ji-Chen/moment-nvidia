...
class NBEATS(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.task_name not in [TASKS.LONG_HORIZON_FORECASTING, TASKS.SHORT_HORIZON_FORECASTING]:
    ...
    def _check_and_process_inputs(self, x_enc):
        assert len(x_enc.size()) == 3, "x_enc must be of shape (batch_size, n_channels, seq_len)"
        assert x_enc.size()[-1] == self.backcast_length, "seq_len of x_enc must be equal to backcast_length"
