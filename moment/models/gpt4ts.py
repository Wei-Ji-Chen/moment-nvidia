...
class GPT4TS(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.randomly_initialize_backbone:
            model_config = GPT2Config.from_pretrained(
                self.transformer_backbone) # Different from fpt.py
        ...
        if self.task_name == TASKS.LONG_HORIZON_FORECASTING or self.task_name == TASKS.SHORT_HORIZON_FORECASTING:
        ...
        elif self.task_name == TASKS.IMPUTATION or self.task_name == TASKS.ANOMALY_DETECTION:
            ...
            raise ValueError(
                f"Unknown task name: {self.task_name}")
    ...
    def forecast(self, x_enc, **kwargs):
        ...
        (1) Long-term-forecasting: XXXX
        (2) In remaining directories: For example in XXXX
        ...
        enc_out = enc_out.permute(0, 2, 1)     # [batch_size x (seq_len+pred_len) x n_channels]
        enc_out = torch.nn.functional.pad(enc_out, (0, 768-enc_out.shape[-1]))
        # [batch_size x (seq_len+pred_len) x d_model]
        
        dec_out = self.gpt2(
            inputs_embeds=enc_out).last_hidden_state  # [batch_size x (seq_len+pred_len) x d_model]
        
        dec_out = dec_out[:, :, :self.d_ff]           # [batch_size x (seq_len+pred_len) x d_ff]
        
        dec_out = self.out_layer(dec_out)             # [batch_size x (seq_len+pred_len) x n_channels]
        dec_out = dec_out.permute(0, 2, 1)            # [batch_size x n_channels x (seq_len+pred_len)]
    ...
    def reconstruct(self, x_enc, mask=None, **kwargs):
        ...
        mask = mask.unsqueeze(1).repeat(1, x_enc.shape[1], 1) # [batch_size x n_channels x seq_len]
        ...
        stdev = torch.sqrt(torch.sum(x_enc * x_enc, dim=2) /
                           torch.sum(mask == 1, dim=2) + 1e-5)
        ...
        dec_out = self.gpt2(
            inputs_embeds=enc_out).last_hidden_state  # [batch_size x seq_len x d_model]
        ...
        return GPT4TSOutputs(reconstruction=dec_out, timeseries=x_enc, mask=mask[:, 0, :])
    ...
    def pretraining(self, 
        ...
        return GPT4TSOutputs(reconstruction=dec_out, timeseries=x_enc, pretrain_mask=mask[:, 0, :])
    ...
    def detect_anomalies(self, 
        ...
        return GPT4TSOutputs(reconstruction=outputs.reconstruction,
