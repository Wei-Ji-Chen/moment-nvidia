...
class AnomalyAttention(nn.Module):
    def __init__(self, win_size, device, mask_flag=True, scale=None, attention_dropout=0.0):
        ...
        b = np.arange(window_size).repeat(window_size).reshape(window_size, window_size).T
        self.distances = torch.from_numpy(np.abs(a - b).astype(np.float32))#.to(self.device)
