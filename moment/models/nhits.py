...
class NHITS(nn.Module): #BaseWindows
    """NHITS
    ...
    **Parameters:**<br>
    ...
    `loss`: PyTorch module, instantiated train loss class from [losses collection](XXXX).<br>
    `valid_loss`: PyTorch module=`loss`, instantiated valid loss class from [losses collection](XXXX).<br>
    ...
    `scaler_type`: str='identity', type of scaler for temporal inputs normalization see [temporal scalers](XXXX).<br>
    `random_seed`: int, random_seed for pytorch initializer and numpy generators.<br>
    `num_workers_loader`: int=os.cpu_count(), workers to be used by `TimeSeriesDataLoader`.<br>
    `drop_last_loader`: bool=False, if True `TimeSeriesDataLoader` drops last non-full batch.<br>
    `alias`: str, optional,  Custom name of the model.<br>
    `**trainer_kwargs`: int,  keyword trainer arguments inherited from [PyTorch Lighning's trainer](XXXX).<br>
    **References:**<br>
    ...
    Accepted at the Thirty-Seventh AAAI Conference on Artificial Intelligence.](XXXX)
    """
    ...
    def __init__(
        ...
        self.h=configs.forecast_horizon
        self.input_size=configs.seq_len
        self.futr_exog_list=configs.futr_exog_list
        self.hist_exog_list=configs.hist_exog_list
        self.stat_exog_list=configs.stat_exog_list
        self.exclude_insample_y=configs.exclude_insample_y
        self.loss=nn.L1Loss() #MAE() #configs.loss, # TODO - use MSE() if possible
        self.valid_loss=configs.valid_loss
        self.max_steps=configs.max_steps
        self.learning_rate=configs.learning_rate
        self.num_lr_decays=configs.num_lr_decays
        self.early_stop_patience_steps=configs.early_stop_patience_steps
        self.val_check_steps=configs.val_check_steps
        self.batch_size=configs.batch_size
        self.windows_batch_size=configs.windows_batch_size
        self.valid_batch_size=configs.valid_batch_size
        self.inference_windows_batch_size=configs.inference_windows_batch_size
        self.start_padding_enabled=configs.start_padding_enabled
        self.step_size=configs.step_size
        self.scaler_type=configs.scaler_type
        self.num_workers_loader=configs.num_workers_loader
        self.drop_last_loader=configs.drop_last_loader
        self.random_seed=configs.random_seed
        self.decompose_forecast = False

        # Architecture    
        # currently read as string instead of None
        self.futr_input_size = 0 #len(configs.futr_exog_list)
        self.hist_input_size = 0 #len(configs.hist_exog_list)
        self.stat_input_size = 0 #len(configs.stat_exog_list)

        blocks = self.create_stack(
            h=configs.forecast_horizon,
            input_size=configs.seq_len,
            stack_types=configs.stack_types,
            ...
            interpolation_mode=configs.interpolation_mode,
            dropout_prob_theta=configs.dropout_prob_theta,
    ...
    def forward(self, x_enc: torch.Tensor, mask: Optional[torch.Tensor] = None, **kwargs):

        x_enc = x_enc.squeeze(1)
        if mask is None:
            insample_y = x_enc
            insample_mask = torch.ones_like(x_enc)
        else:
            insample_y = x_enc
            insample_mask = mask
        ...
        if self.decompose_forecast:
            ...
            return NHITSOutputs(backcast=x_enc, forecast=forecast, timeseries=x_enc)
        else:
            return NHITSOutputs(backcast=x_enc, forecast=forecast, timeseries=x_enc)